...
INSERT INTO user_relations (user_id, relation_user_id, kind) VALUES ($1, $2, $3);

-- name: GetAllRelations :many
SELECT u.*
FROM users u
INNER JOIN user_relations ur ON ur.relation_user_id = u.id
WHERE ur.user_id = $1 AND kind = $2;

-- name: DeleteRelation :exec
DELETE FROM user_relations WHERE user_id = $1 AND relation_user_id = $2 AND kind = $3;
...
SELECT EXISTS ( SELECT 1 FROM user_relations WHERE user_id = $1 AND relation_user_id = $2 AND kind = $3 );
...
INSERT INTO auth_verification (user_id, kind, token, expiry) VALUES ($1, $2, $3, $4);

-- name: GetAuthVerification :one
SELECT * FROM auth_verification WHERE user_id = $1 AND kind = $2;

-- name: DeleteAuthVerification :exec
DELETE FROM auth_verification WHERE user_id = $1 AND kind = $2;
...
SELECT EXISTS (
    ...
    WHERE ur.user_id = $1 AND m.id = $2 AND kind = $3
);
...
-- name: DeleteAllNotificationsByKind :exec
DELETE FROM notifications WHERE user_id = $1 AND kind = $2;
