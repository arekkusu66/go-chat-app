...
func CreateUser(ctx context.Context, DB *sql.DB, Query *Queries, userParams *CreateUserParams) error {
...
func AddFriend(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
	...
	if err != nil {
		return http.StatusBadRequest, errors.New("couldnt check the relation Kind with this user")
	}
...
func CancelFriendRequest(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
	sentFriendReq, err := Query.CheckRelation(ctx, CheckRelationParams{
		UserID: userId,
		RelationUserID: targetId,
		Kind: types.SENT_FRIEND_REQ,
	...
	if err != nil {
		return http.StatusInternalServerError, errors.New("couldnt check the relation Kind with this user")
	}
...
func AcceptFriendRequest(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
	isBlocked, err := Query.CheckMutualRelation(ctx, CheckMutualRelationParams{
		UserID: userId,
		RelationUserID: targetId,
		Kind: types.BLOCKED_USER,
...
func IgnoreFriendRequest(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
	receivedFriendReq, err := Query.CheckRelation(ctx, CheckRelationParams{
		UserID: userId,
		RelationUserID: targetId,
		Kind: types.RECEIVED_FRIEND_REQ,
...
func BlockUser(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
	tx, err := DB.BeginTx(ctx, nil)
...
func UnblockUser(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
...
func CreateChatroom(ctx context.Context, DB *sql.DB, Query *Queries, userId uuid.UUID, title string) error {
	tx, err := DB.BeginTx(ctx, nil)
...
func SendDM(ctx context.Context, DB *sql.DB, Query *Queries, userId, targetId uuid.UUID) (int, error) {
	isBlocked, err := Query.CheckMutualRelation(ctx, CheckMutualRelationParams{
		UserID: userId,
		RelationUserID: targetId,
		Kind: types.BLOCKED_USER,
...
func DeleteMessage(ctx context.Context, DB *sql.DB, Query *Queries, id int64) error {
	tx, err := DB.BeginTx(ctx, nil)
