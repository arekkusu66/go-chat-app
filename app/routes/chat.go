...
func CreateChatH(app *app.App) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		createdChatsCount, _ := app.Query.GetCreatedChatroomsCount(r.Context(), user.ID)
		...
		if err := db.CreateChatroom(r.Context(), app.DB, app.Query, user.ID, title); err != nil {
...
func ChatH(app *app.App) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		user, _ := app.Query.GetUserById(r.Context(), userId)
		...
		chatroomExists, _ := app.Query.CheckChatroomExistence(r.Context(), intId)
		...
		chatroom, _ := app.Query.GetChatroom(r.Context(), intId)

		pages.Chat(app, user, chatroom).Render(r.Context(), w)
	})
}


func ChatActionsH(app *app.App) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		userId, status, err := app.GetUserID(r, w)
		...
		alreadyJoined, _ := app.Query.CheckIfAlreadyJoined(r.Context(), db.CheckIfAlreadyJoinedParams{
			UserID: userId,
			ChatroomID: intId,
		})
		...
		switch r.URL.Query().Get("action") {
			case "join":
				...
				app.Query.JoinChatroom(r.Context(), db.JoinChatroomParams{
					ChatroomID: intId,
					UserID: userId,
				})
			...
			case "leave":
				...
				app.Query.LeaveChatroom(r.Context(), db.LeaveChatroomParams{
					UserID: userId,
					ChatroomID: intId,
				})
...
func GetOptionsH(app *app.App) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		userId, status, err := app.GetUserID(r, w)
		...
		message, err := app.Query.GetMessageById(r.Context(), intId)
...
func GetMessageH(app *app.App) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		userId, status, err := app.GetUserID(r, w)
		...
		isReply, _ := app.Query.CheckIfMessageIsReply(r.Context(), db.CheckIfMessageIsReplyParams{
			ID: intId,
			UserID: userId,
		})

		isBlocked, _ := app.Query.CheckMessageUserRelation(r.Context(), db.CheckMessageUserRelationParams{
			ID: intId,
			UserID: userId,
		...
		datas, err := app.Query.GetFullMessageDatas(r.Context(), intId)
